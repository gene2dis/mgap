...
class SampleSheetGenerator:
    ...
    def process_illumina(self, files):
        ...
        for sample_name, reads in sorted(samples_dict.items()):
            ...
            for i, r1 in enumerate(r1_files):
                ...
                self.samples.append({"sample": sample_name, "fastq_1": r1, "fastq_2": r2})
    ...
    def process_ont(self, files):
        ...
        for filepath in sorted(files):
            ...
            self.samples.append({"sample": sample_name, "fastq_1": str(filepath.absolute()), "fastq_2": ""})
    ...
    def process_contigs(self, files):
        ...
        for filepath in sorted(files):
            ...
            self.samples.append({"sample": sample_name, "fasta": str(filepath.absolute())})
...
def parse_args(argv=None):
    ...
    parser.add_argument("input_dir", type=Path, help="Directory containing sequencing files (FASTQ or FASTA)")
    parser.add_argument("output_file", type=Path, help="Output samplesheet CSV file")
    parser.add_argument(
        "-t",
        "--type",
        dest="data_type",
...
def main(argv=None):
    ...
    generator = SampleSheetGenerator(input_dir=args.input_dir, output_file=args.output_file, data_type=args.data_type)
